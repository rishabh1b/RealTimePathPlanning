...
void SMP::addNode(Nodes n, Data & tree)
{
	tree = *insert(&tree, n);
}

Nodes * SMP::nearestNode(Nodes n, Data *tree)
{
	return nearSearch(tree, n);
}
//
...
void RRTstar::nextIter(Data &tree, const std::list<obstacles> obst, Nodes * u_)
{
	Nodes u;
	if (u_ == NULL)
		u = SMP::sampler();
	...
	Nodes* v = SMP::nearestNode(u, &tree);
	...
	std::list<Nodes*> closestNeighbours;
	closestNeighbours = this->findClosestNeighbours(u, rrtstarradius, &tree);
	//if (closestNeighbours.empty()) {
	//	closestNeighbours.push_back(v);
	//}
	if (closestNeighbours.empty()) return;
...
std::list<Nodes*> RRTstar::findClosestNeighbours(Nodes n, float radius, Data *tree)
{
	std::queue<Data*> line;
	std::list<Nodes*>  list;
	if (tree != NULL) {
	...
	while (!list.empty()) {
		Data *temp = line.front();
		if (temp->node.location.distance(n.location) < radius) {
			list.push_back(&temp->node);
		}
		line.pop();
		if (temp->left != NULL) {
			line.push(temp->left);
